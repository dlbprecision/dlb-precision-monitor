...
**[Download DLB Precision Monitor for Windows 11](https://github.com/dlbprecision/dlb-precision-monitor/releases/download/v0.1.3/DLB-Precision-Monitor-0.1.3-Setup.exe)**

**v0.1.3 fixes clipped sensor details in Settings.** Long diagnostics wrap and remain accessible in the scrolling content area. Setup, the uninstaller and DLB's application files carry DLB Precision, LLC signatures and timestamps. It remains a prerelease for testing. No GitHub account is needed. Download only the setup EXE; the `.sha256` file on the [release page](https://github.com/dlbprecision/dlb-precision-monitor/releases/tag/v0.1.3) is an optional checksum, not another installer.
...
.\scripts\Build-Installer.ps1 -Version 0.1.3 -OutputDirectory .\artifacts\unsigned-0.1.3-attempt-1
```
