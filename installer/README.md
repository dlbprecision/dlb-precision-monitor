...
```powershell
powershell -NoProfile -ExecutionPolicy Bypass -File .\scripts\Build-Installer.ps1 -Version 0.1.4 -OutputDirectory .\artifacts\unsigned-0.1.4-attempt-1
```
...
- Reuses PawnIO when its kernel service is running and the driver device can be opened with the access used by LibreHardwareMonitor. Neither `PawnIOLib.dll` nor an uninstall registry entry is required for readiness; LibreHardwareMonitor 0.9.6 accesses the driver directly. This check establishes driver accessibility, not sensor accuracy or support for every device.
- First tries to start a stopped existing PawnIO driver. If it remains unavailable or is absent, setup uses the bundled, unmodified signed 2.2.0 installer with `-install -silent`. It does not invoke an unsupported repair switch or automatically uninstall the shared driver. If uninstall metadata identifies a version newer than 2.2.0, setup preserves it and reports that it could not be made available instead of invoking the older bundle.
- Handles driver exit **3010** as reboot required. If the driver is already accessible, setup can finish and request a restart; otherwise it stops before installing DLB files and asks the user to restart and rerun the same DLB installer. No separate driver download is needed. Other vendor failures include the exit code and direct the user to the setup log if a restart and retry do not resolve them.
- Stops the DLB sensor service before prerequisite installation or file updates, and restarts it after configuration. Cancelling or failing setup preparation restarts a previously running DLB service when setup exits. Inno Restart Manager handles app files in use.
